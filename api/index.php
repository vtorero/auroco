...
$db = new mysqli("localhost","kvconsul_auroco","auroco2023","kvconsul_auroco");
...
$app->get("/facturados",function() use ($app,$db){
...
$app->get("/monedas",function() use($db,$app){
    ...
    $resultado = $db->query("SELECT * FROM kvconsul_auroco.MONEDAS order by idMONEDAS desc");
...
$app->post("/buscafacturados",function() use($db,$app){
    $json = $app->request->getBody();
   $data = json_decode($json, false);
...
$sql="SELECT f.id,CONCAT(f.serie,f.correlativo) AS C_ORDEN,f.agencia,C.RAZON_SOCIAL,C.RUC AS C_RUC,f.fecha AS F_CREACION,f.total AS TOTAL,f.estado AS EJECUTIVO,f.mensaje AS MOTIVO  FROM facturas f, ORD_ORDENES O,ORD_CLIENTES C WHERE f.ord_orden=O.C_ORDEN AND O.C_CLIENTE=C.C_CLIENTE";


if($data->C_CLIENTE!="0"){
    $sql.=" AND C.C_CLIENTE='{$data->C_CLIENTE}' ";
}


if($data->INICIO_VIGENCIA!="" && $data->FIN_VIGENCIA!="") {
    $sql.=" AND f.fecha BETWEEN '{$inicio}' and '{$fin}'";
}
...
$sql="SELECT T.ID,T.C_CONTRATO,T.NRO_FISICO,C.C_CLIENTE,C.RAZON_SOCIAL,C.RUC AS C_RUC,T.INICIO_VIGENCIA,T.F_CREACION,T.FIN_VIGENCIA,T.OBSERVACIONES,T.C_MONEDA,T.INVERSION TOTAL,T.MONTO_ORDENAR ,TIPO_CAMBIO 
FROM ORD_CONTRATOS T,ORD_CLIENTES C
WHERE T.C_CLIENTE=C.C_CLIENTE  ";
...
if($data->INICIO_VIGENCIA!="" && $data->FIN_VIGENCIA!="" && $data->C_CONTRATO=="") {
    $sql.=" AND T.INICIO_VIGENCIA BETWEEN '{$inicio}' and '{$fin}'";
}

if($data->C_CONTRATO!=""){
    $sql.=" AND T.C_CONTRATO LIKE '%{$data->C_CONTRATO}%'";
}

$sql.=" ORDER  by T.F_CREACION DESC";
...
//$cadena="INSERT INTO `kvconsul_auroco`.`ORD_EJECUTIVOS`    (`C_EJECUTIVO`,    `DNI_EJECUTIVO`,    `NOMBRES`,    `F_CREACION`,    `USUARIO`   ) VALUES ";

//clientes

$cadena="INSERT INTO `kvconsul_auroco`.`ORD_CLIENTES` ( `C_CLIENTE`, `RAZON_SOCIAL`, `CONTACTO`, `RPT_LEGAL`, `RPT_DNI`, `RPT_DIRECCION`, `RUC`, `DIRECCION`, `TELEFONO`, `F_CREACION`, `USUARIO`) VALUES ";

//medios

//$cadena="INSERT INTO `kvconsul_auroco`.`ORD_MEDIOS` (`ID`, `C_MEDIO`, `TIPO`, `NOMBRE`,`DESCRIPCION`,`F_CREACION`,`C_USUARIO_CREACION`) VALUES";

//programas

//$cadena="INSERT INTO `kvconsul_auroco`.`ORD_PROGRAMAS_AUT` (`ID`, `PROGRAMA`, `REGION`,`CANAL`,`GENERO`,`TEMA`,`PERIODO`,`DIAS`,`RATING`,`MILES`,`COSTO`,`F_CREACION`,`C_USUARIO`) VALUES";

//contratos

//$cadena="INSERT INTO `kvconsul_auroco`.`ORD_CONTRATOS` (`C_CONTRATO`,`C_CLIENTE`,`INICIO_VIGENCIA`,`FIN_VIGENCIA`,`NRO_FISICO`,`C_MONEDA`,`INVERSION`,`INVER_IGV`,`MONTO_ORDENAR`,`MONTO_ORD_IGV`,`TIPO_CAMBIO`,`TASA_IGV`,`OBSERVACIONES`,`C_USUARIO`,`F_CREACION`) VALUES";
...
$app->get("/clientes",function() use ($app,$db){
   ...
   $resultado = $db->query("SELECT * FROM kvconsul_auroco.ORD_CLIENTES order by RAZON_SOCIAL ASC;");
...
$app->get("/ordenprint/:id",function($id) use ($app,$db){
    ...
    $resultado = $db->query("SELECT ORD.ID,ORD.C_ORDEN,ORD.REVISION,ORD.NOMBRE,ORD.MES_VIGENCIA,ORD.INICIO_VIGENCIA,ORD.FIN_VIGENCIA,ORD.C_MONEDA,ORD.RUC,ORD.RAZON_SOCIAL,ORD.PRODUCTO,ORD.MOTIVO,ORD.DURACION,ORD.PROGRAMA,ORD.DIAS,CASE WHEN ORD.DIAS ='S-D' THEN 4 WHEN  ORD.DIAS = 'D' THEN 3 WHEN ORD.DIAS = 'S' THEN 2 WHEN ORD.DIAS = 'L-V' THEN 1  END  DIA, ORD.PERIODO,ORD.TEMA,ORD.C_MEDIO,ORD.INVERSION_TOTAL,ORD.OBSERVACIONES,
...
(select O.C_ORDEN,CL.RUC,CL.RAZON_SOCIAL,O.C_MEDIO,M.NOMBRE,O.OBSERVACIONES,
 ...
 O.FIN_VIGENCIA,
    ...
    ORD.FIN_VIGENCIA,
         ...
         ORD.INVERSION_TOTAL ORDER BY DIA,PERIODO;");
...
//enviar facturas
$app->post("/facturar",function() use ($app,$db){
 $json = $app->request->getBody();
   $data = json_decode($json, true);
   echo  json_encode($data);
  ...
  /* try {
    ...
    $sql="call SP_FACTURAR('{$data->DNI_EJECUTIVO}','{$data->NOMBRES}','{$data->USUARIO}')";
...
//listar contratos para facturar
$app->get("/facturar",function() use ($app,$db){
   ...
   $resultado = $db->query("SELECT T.ID,T.C_CONTRATO,T.NRO_FISICO,C.C_CLIENTE,C.RAZON_SOCIAL,C.RUC AS C_RUC,T.INICIO_VIGENCIA,T.F_CREACION,T.FIN_VIGENCIA,T.OBSERVACIONES,T.C_MONEDA,T.INVERSION TOTAL,T.MONTO_ORDENAR ,TIPO_CAMBIO 
FROM ORD_CONTRATOS T,ORD_CLIENTES C
WHERE T.C_CLIENTE=C.C_CLIENTE  ORDER  by T.F_CREACION DESC limit 80");
...
$app->post("/contrato_cliente",function() use ($app,$db){
   ...
   $sql="SELECT * FROM kvconsul_auroco.ORD_CONTRATOS  WHERE  C_CLIENTE='{$data['C_CLIENTE']}' order by C_CONTRATO ASC";
...
$app->post("/contratos_clientes",function() use ($app,$db){
   ...
   $sql="SELECT * FROM kvconsul_auroco.ORD_CONTRATOS  WHERE  C_CLIENTE='{$data['C_CLIENTE']}' AND INICIO_VIGENCIA<= CURDATE() AND FIN_VIGENCIA>=	CURDATE() order by C_CONTRATO ASC";
...
$app->get("/contrato_detalle/:id",function($id) use($db,$app){
   ...
   $sql="SELECT ID,C_CONTRATO,C_CLIENTE,date_format(INICIO_VIGENCIA,'%d/%m/%Y') INICIO_VIGENCIA,date_format(FIN_VIGENCIA,'%d/%m/%Y') FIN_VIGENCIA,    NRO_FISICO,   C_MONEDA, INVERSION,INVER_IGV,MONTO_ORDENAR,MONTO_ORD_IGV,TIPO_CAMBIO,TASA_IGV,OBSERVACIONES,C_USUARIO,F_CREACION  ,(SELECT SALDO_ACTUAL FROM ORD_MOVIMIENTO_SALDOS WHERE C_CONTRATO='{$id}' ORDER BY N_MOVIMIENTO DESC LIMIT 1) SALDO_ACTUAL FROM kvconsul_auroco.ORD_CONTRATOS WHERE C_CONTRATO='{$id}'";
...
$app->get("/medio_programas/:medio",function($medio) use($db,$app){
   ...
   $data = json_decode($json, TRUE);
    ...
    $sql="SELECT p.ID,PROGRAMA,p.PERIODO TEMA FROM  kvconsul_auroco.ORD_PROGRAMAS_AUT p inner join kvconsul_auroco.ORD_MEDIOS m where  m.C_MEDIO='{$medio}' AND p.ESTADO='SI' AND m.NOMBRE=p.CANAL ORDER BY PROGRAMA;";
...
$app->get("/programa/:id",function($id) use($db,$app){
   ...
   $sql="SELECT * FROM  kvconsul_auroco.ORD_PROGRAMAS_AUT where  ID='{$id}' ORDER BY PROGRAMA";
